...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-20 items-center justify-between hidden w-full h-16 px-3 bg-alt bg-opacity-40 md:flex backdrop-blur backdrop-filter firefox:bg-opacity-90">
      <Link href="/">
        <Image
          src={logo}
          alt="logo"
      ...
      <div className="flex items-center h-full space-x-2">
        {user ? (
          <>
            <div
              className="flex items-center justify-between w-24 h-8 pl-2 pr-1 transition ease-in-out rounded-full cursor-pointer bg-main hover:bg-opacity-80"
